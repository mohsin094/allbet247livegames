<template>
  <div>
    <div class="row">
      <div class="col">
        <button
          class="btn btn-golden text-dark mt-3"
    ...
    <div class="row mt-3">
      <div class="col-md-6">
        ...
        <div
          class="bg-dark-gradient text-center p-4 simple-page overflow-box position-relative mb-3"
        ...
        >
            ...
            <tbody>
              <tr v-for="ticket in tickets" @click="getTicket(ticket._id)">
                ...
                <td>
                  <strong
                    :class="[
                      { 'text-grass': ticket.status == 'open' },
                      { 'text-red': ticket.status == 'closed' },
                      { 'text-golden-light': ticket.status == 'user_pending' },
                    ]"
                  >
                    {{ ticket.status }}
                  </strong>
          ...
          <p v-else class="vertical-center notif-text">
      ...
      <div class="col-md-6">
        ...
        <div
          class="bg-dark-gradient p-4 simple-page overflow-box position-relative msgs-wrapper"
        >
        ...
        <div class="input-group" style="height: 50px">
          <input
            v-model="rplyTxt"
            :disabled="messages.length == 0 ? 'disabled' : false"
            @keyup.enter="sendMsg()"
            type="text"
          ...
          <div class="input-group-append">
            <button
              @click="sendMsg()"
              :disabled="messages.length == 0 ? 'disabled' : false"
              class="btn btn-outline-secondary msg-input"
              id="send-msg"
              type="button"
              style="height: 50px"
            >
...
export default {
  ...
  methods: {
    sendMsg() {
      ...
      this.$axios
        ...
        .then(function (response) {
          ...
          const now = new Date();
          const day = now.getHours() + ":" + now.getMinutes();
    ...
    getTicketList() {
      const url =
        import.meta.env.VITE_BACKEND_BASE_URL + "/ticket/default/list";
      let instance = this;
