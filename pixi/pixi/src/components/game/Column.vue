<template>
	<div :class="{'top-column': (data.index < 12), 'bottom-column': (data.index > 11)}" class="column-wrapper" v-if="data" :style="{top: (data.index < 12) ? data.y+'px' : 'initial',bottom: (data.index > 11 ) ? data.y+'px' : 'initial' , left: data.x+'px', width: globalVars.checkerSize+'px'}">
		...
		<ul class="column-holder">
			<li :style="{'margin-top': (data.index < 12) ? '-50%' : 'initial','margin-bottom': (data.index > 11 ) ? '-50%' : 'initial'}" @click="$emit('touch', checker.index)" v-for="checker in data.container" :key="checker.index">
